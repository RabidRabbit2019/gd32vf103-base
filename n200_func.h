...
uint32_t __attribute__((noinline)) measure_cpu_freq(uint32_t n);

/* ECLIC relevant functions */
void eclic_init ( uint32_t num_irq );
uint64_t get_timer_value(void);

__attribute__ ((always_inline)) inline void reset_timer_value() {
  *(volatile uint32_t *)(TIMER_CTRL_ADDR + TIMER_MTIME + 4) = 0;
  *(volatile uint32_t *)(TIMER_CTRL_ADDR + TIMER_MTIME) = 0;
  *(volatile uint32_t *)(TIMER_CTRL_ADDR + TIMER_MTIME + 4) = 0;
};
__attribute__ ((always_inline)) inline void set_timer_cmp_value( uint64_t a_value ) {
  *(volatile uint64_t *)(TIMER_CTRL_ADDR + TIMER_MTIMECMP) = a_value;
};
